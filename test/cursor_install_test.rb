...
class CursorInstallTest < Minitest::Test
  ...
  def test_gemspec_excludes_cursor_boot_files
    spec = Gem::Specification.load(File.join(ROOT, "recording_studio_notifications_email.gemspec"))

    assert spec, "recording_studio_notifications_email.gemspec failed to load"
    assert_empty spec.files.grep(%r{\A\.cursor/})
  end
